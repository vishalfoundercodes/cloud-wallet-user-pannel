...
import { Clock, CircleCheckBig, X, ArrowLeft, CloudSnow } from "lucide-react";
...
const TotalWithdrawalTransition = () => {
  ...
  const fetchTransactions = async () => {
    try {
      const res = await axios.get(`${apis?.Transaction_history}?user_id=${userIdHai}&type=2`);
      console.log("approved transesitiobddv", res)
      if (res.data?.success) {
